...
results = modelUNet.fit(x_train, y_train, validation_split=0.1, batch_size=8, epochs=50, callbacks=callbacks)
...
for i in range(len(x_val)):
    img = x_val[i]
    predMask = modelUNet.predict(np.expand_dims(img, axis=0), verbose=0)
    predMask = np.squeeze(predMask)  # Remove the batch dimension
    
   
    filename = f'mask_{i}.png'
    filepath = os.path.join(output_folder, filename)
    plt.imsave(filepath, predMask, cmap='gray')
    

filenames = os.listdir(output_folder)
random_filenames = random.sample(filenames, 9)
...
for i, filename in enumerate(random_filenames):
    filepath = os.path.join(output_folder, filename)
    img_pred = plt.imread(filepath)
    img_orig = x_val[i]  
    row = i // 3
    col = i % 3
    axs[row, col*2].imshow(img_orig)
    axs[row, col*2].axis('off')
    axs[row, col*2+1].imshow(img_pred, cmap='gray')
